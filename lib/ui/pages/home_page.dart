...
class _HomePageState extends State<HomePage> {
  ...
  Widget build(BuildContext context) {
    ...
    Future<void> incrementBegenenler(int index, String ownerId) async {
      final FirebaseFirestore _firestore = FirebaseFirestore.instance;
      ...
      try {
        ...
        // Get the current begenenler list

        print(userSnapshot[0]['begenenler'].length.toString());

        // Increment the count

        // Update Firestore with the modified begenenler list
      } catch (e) {
        print('Error incrementing begenenler: $e');
      }
    }

    getYorumlar(String ownedId) async {
      DocumentSnapshot userSnapshot = await FirebaseFirestore.instance
          .collection('users')
          .doc(ownedId)
          .get();
    ...
    Future<void> yorumEkle(String yorum, String ownerId) async {
      DocumentSnapshot userSnapshot = await FirebaseFirestore.instance
          .collection('users')
          .doc(userId)
          .get();
                                ...
                                child: Column(
                                  children: [
                                    GestureDetector(
                                      onTap: () {
                                        String ownerId = userDocument.id;
                                        incrementBegenenler(index, ownerId);
                                      },
                                      child: Container(
                                        height: 40,
                                        width: 40,
